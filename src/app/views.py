...
def hire_car(request):
    if request.method == "POST":
        serializer = CarHiringSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save(status="Pending")  # Default to Pending
    ...
    serializer = CarHiringSerializer(bookings, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)
...
# Get a user's own bookings with nested car details
@api_view(["GET"])
...
def user_bookings(request):
    ...
    carhires_serializer = CarHiringSerializer(carhires, many=True)
...
# Delete a user's own booking safely
@api_view(["DELETE"])
@permission_classes([IsAuthenticated])
...
def simulate_payment(request):
    ...
    if booking_type == "carhire":
        ...
        serializer = CarHiringSerializer(booking)
    elif booking_type == "carwash":
